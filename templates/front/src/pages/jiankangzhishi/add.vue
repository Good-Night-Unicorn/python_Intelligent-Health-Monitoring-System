<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="80px"
			>
			<el-form-item class="add-item" label="知识标题" prop="zhishibiaoti">
				<el-input v-model="ruleForm.zhishibiaoti" 
					placeholder="知识标题" clearable :disabled=" false  ||ro.zhishibiaoti"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`jiankangzhishi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jiankangzhishi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
