	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="''">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yinshijianyi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yinshijianyi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yinshijianyi','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="饮食建议" name="first">
					<div class="ql-snow ql-editor" v-html="detail.yinshijianyi"></div>
				</el-tab-pane>
			</el-tabs>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yinshijianyi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yinshijianyiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此饮食建议？') .then(_ => {
					this.$http.post('yinshijianyi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
