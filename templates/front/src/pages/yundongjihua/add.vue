<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="计划时间" prop="jihuashijian">
				<el-date-picker
			...
			<el-form-item class="add-item" label="运动计划" prop="yundongjihua">
				<editor 
					v-model="ruleForm.yundongjihua" 
					class="editor" 
					myQuillEditor="yundongjihua"
					action="file/upload">
				</editor>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			...
			this.ruleForm.jihuashijian = this.getCurDate()
		},
		methods: {
			getMakeZero(s) {
				return s < 10 ? '0' + s : s;
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='yundongjihua'){
							this.ruleForm.yundongjihua = obj[o];
							this.ro.yundongjihua = true;
							continue;
						}
					}
				}else if(type=='edit'){
			...
			// 多级联动参数
			info() {
				this.$http.get(`yundongjihua/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`yundongjihua/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
