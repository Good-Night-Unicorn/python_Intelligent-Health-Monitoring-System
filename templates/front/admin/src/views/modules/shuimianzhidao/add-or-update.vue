	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="指导时间" prop="zhidaoshijian" >
					<el-date-picker
				...
				<el-form-item class="input" v-else-if="ruleForm.zhidaoshijian" label="指导时间" prop="zhidaoshijian" >
					<el-input v-model="ruleForm.zhidaoshijian" placeholder="指导时间" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item v-if="type!='info'"  label="睡眠指导" prop="shuimianzhidao" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.shuimianzhidao" 
					class="editor"
					myQuillEditor="shuimianzhidao"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.shuimianzhidao" label="睡眠指导" prop="shuimianzhidao" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.shuimianzhidao"></span>
			</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='shuimianzhidao'){
							this.ruleForm.shuimianzhidao = obj[o];
							this.ro.shuimianzhidao = true;
							continue;
						}
					}
				}
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.shuimianzhidao = this.ruleForm.shuimianzhidao.replace(reg,'../../../djangov5gemqq6/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shuimianzhidao/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shuimianzhidaoCrossAddOrUpdateFlag = false;
											this.parent.search();
