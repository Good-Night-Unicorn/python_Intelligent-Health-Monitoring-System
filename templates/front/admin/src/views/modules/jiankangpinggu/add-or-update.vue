	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="健康评估" prop="jiankangpinggu" >
					<el-select :disabled="ro.jiankangpinggu" v-model="ruleForm.jiankangpinggu" placeholder="请选择健康评估" >
						<el-option
				...
				<el-form-item v-else class="input" label="健康评估" prop="jiankangpinggu" >
					<el-input v-model="ruleForm.jiankangpinggu"
						placeholder="健康评估" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-else-if="ruleForm.pingguneirong" label="评估内容" prop="pingguneirong" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.pingguneirong"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.pingguneirong = this.ruleForm.pingguneirong.replace(reg,'../../../djangov5gemqq6/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
