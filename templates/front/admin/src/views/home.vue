<template>
	<div class="home-content">
		...
		<div class="statis-box">
			...
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('jiankangzhishi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{jiankangzhishiCount}}</div>
					<div class="name">健康知识总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<div class="type3">
			...
			<div id="jiankangzhishiChart1" class="echarts3 animate__animated" v-if="isAuth('jiankangzhishi','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.getjiankangzhishiCount();
		if(this.isAuth('jiankangzhishi','首页统计')){
			this.jiankangzhishiChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'jiankangzhishiChart1',css:'animate__fadeInUp'},
			]
...
// 1234 饼2
		...
		getjiankangzhishiCount() {
			this.$http({
				...
				if (data && data.code == 0) {
					this.jiankangzhishiCount = data.data
				}
			})
		},
// 1234 饼2
		jiankangzhishiChat1() {
			this.$nextTick(()=>{
				...
				var jiankangzhishiChart1 = echarts.init(document.getElementById("jiankangzhishiChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						jiankangzhishiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							jiankangzhishiChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
...
<style lang="scss" scoped>
	.home-content {
		padding: 10px 30px;
		background: url(http://codegen.caihongy.cn/20250325/4963da5a718444139d196cc6e01b9702.png) no-repeat center top / cover;
		display: flex;
		width: 100%;
		min-height: 100vh;
		flex-wrap: wrap;
		...
		.statis-box {
			...
			.statis1 {
				...
				padding: 10px;
				margin: 0 0 10px;
				background: url(http://codegen.caihongy.cn/20241007/83001f4cd91743b1ba49d591dfc95020.png) center center/cover no-repeat;
				display: flex;
				width: 100%;
				transition: 0.3s;
				.left {
